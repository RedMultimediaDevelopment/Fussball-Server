...
class Controller {
  ...
  // Constructor takes:
  // - routePath which is the base path for each service exposed by the controller subclass (ie. '/auth/users')
  ...
  // Controller subclass. Base implementation does nothing but here you should
  ...
  // and the function and register it along with the base path of the route.
	registerServices() {
		var controller_services = this.services;
		Object.keys(controller_services).forEach( full_path => {
			// This is the name of the JS function which implement the service's logic
			var service_function = controller_services[full_path];
			...
			// bind to Express's controller logic
